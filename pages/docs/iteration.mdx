...
It takes a `SimplePath`, or anything that has one &mdash; `Line`, `Rect`, `RegularPolygon`, `Star` and `Spiral` all do. Pass `inclusive: false` for a closed path, where the last point would otherwise repeat the first.
...
The underlying measurements are on `SimplePath` itself (`length`, `pointAt`, `tangentAt`, `pointsAlong`) if you want the points without drawing; see [Paths and Curves](/docs/paths).
