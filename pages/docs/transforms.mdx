...
- `type: "rotational"` (the default) &mdash; `n` copies, evenly spaced around a full turn
- `type: "mirror"` &mdash; a reflected pair; `axis` is `"vertical"` (the default, flipping left to right) or `"horizontal"`
- `type: "kaleidoscope"` &mdash; `n` rotations, each drawn twice, once reflected, so 2n copies
- `at` &mdash; the centre of the symmetry, the middle of the canvas unless you say otherwise
