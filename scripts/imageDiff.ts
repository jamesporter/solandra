...
export type CompareResult = {
  ...
  /** Of those, the ones not explained by a sub pixel shift. */
...
/**
 * Perceptual distance between two pixels, normalised to 0-1. Based on the
...
export function colorDelta(
  ...
  const ar = onWhite(a[ai], aAlpha)
  const ag = onWhite(a[ai + 1], aAlpha)
  const ab = onWhite(a[ai + 2], aAlpha)

  const br = onWhite(b[bi], bAlpha)
  const bg = onWhite(b[bi + 1], bAlpha)
  const bb = onWhite(b[bi + 2], bAlpha)
...
export function compareImages(
  ...
  const { colorTolerance, shiftTolerance, threshold } = {
